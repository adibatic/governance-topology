...
\paragraph{Postulate 1 (The Institutional Triad).}
...
\paragraph{Postulate 2 (Institutional Magnitude and Imbalance).}
From the triad, we derive two fundamental geometric coordinates: \textit{Magnitude} ($M$), representing mean systemic capacity, and \textit{Imbalance} ($I$), representing the structural skew (Euclidean distance from the identity line):
\begin{equation}
    M(s) = \frac{1}{3} \sum_{i=1}^{3} x_i, \quad I(s) = \sqrt{\frac{1}{3} \sum_{i=1}^{3} (x_i - M)^2}.
\end{equation}
...
Holding $M$ constant, institutional imbalance ($I$) is structurally associated with functional development. We hypothesize that asymmetrical institutional growth correlates with an ``Imbalance Premium'' in societal outcomes.
...
Variables within each core dimension are equally weighted to calculate the dimensional coordinates. While dynamic component weighting (e.g., via PCA loadings) was considered, equal weighting was retained for strict theoretical parsimony and to preserve the symmetrical geometry of the manifold.

To evaluate the structural independence of these dimensions, an unconstrained 3-com\-po\-nent Principal Component Analysis (PCA) was conducted on the raw indicators. The unconstrained model revealed functional, collinear groupings rather than clean conceptual ones; for instance, foundational rule of law naturally clustered with health equality, reflecting a generalized ``wealth and capacity'' latent factor. Rather than relying on these purely data-driven groupings, this study deliberately enforces the theoretically constrained Triad (Postulate 1). This theoretical constraint prevents the manifold from collapsing into a simple proxy for baseline wealth, allowing us to explicitly test the geometric trade-offs between classic Mar\-shal\-li\-an and Dah\-li\-an dimensions of governance.
...
In this coordinate system, the manifold is centered at the global sample mean ($0,0$). Consequently, a positive Magnitude ($x > 0$) indicates systemic capacity above the sample average, while the Imbalance axis ($y$) captures deviations from the sample's primary structural trade-off.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{../figures/fig1_pca_manifold.eps}
    \caption{\textbf{The Continuous Manifold of State Governance.} Two-dimensional PCA projection of $N=66$ nation-states. The horizontal axis represents overall Institutional Magnitude, derived from the first principal component (PC1 orientation inverted for intuitive capacity scaling). The vertical axis represents Structural Imbalance (PC2). Labeled anchor states illustrate that diverse geopolitical regimes exist on a continuous mathematical spectrum rather than within discrete categorical clusters.}
...
Despite this continuity, the manifold reveals distinct topological ``neighborhoods'' corresponding to established geopolitical families. A tight cluster of advanced industrial democracies (e.g., Germany, Japan, UK) occupies the high-magnitude, low-skew region. Conversely, large-scale asymmetric powers such as China, Russia, and Iran gravitate toward a shared region of high structural imbalance, while major developing democracies (India, Brazil, Indonesia) occupy a distinct, third quadrant. The fact that these families emerge naturally from the geometric coordinates without categorical labeling provides strong empirical support for the construct validity of the Triad model.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{../figures/fig3_umap_projection.eps}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{../figures/fig4_silhouette_scores.eps}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{../figures/fig5_funnel_capacity.eps}
...
To ensure these findings were not artifacts of the K-Nearest Neighbors imputation process, a strict complete case analysis was conducted as a robustness check. The models were re-run strictly on the subset of nations with perfectly complete baseline data ($N=30$). Even with a drastically reduced sample size, the core findings remained remarkably robust. The ``Imbalance Premium'' held high statistical significance ($\beta = 1.295, p = 0.003$), and model diagnostics remained exceptionally clean ($R^2 = 0.592$, Durbin-Watson = $1.94$). This confirms that the observed geometric correlation between structural asymmetry and societal prosperity is a genuine topological feature of the data, rather than an algorithmic ghost.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{../figures/fig6_residuals.eps}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{../figures/fig7_hdi_map.eps}
